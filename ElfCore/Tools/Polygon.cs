...
namespace ElfCore.Tools
{
	...
	public class PolygonTool : ShapeBase, ITool
	{
		...
		protected override void AttachEvents(bool attach)
		{
			...
			if (attach)
			{
				BtnDrawAsStar.Click += new EventHandler(IsStar_Click);
				NumPoints.Leave += new EventHandler(NumPoints_Leave);
				StartAngle.Leave += new EventHandler(StartAngle_Leave);
			}
			else
			{
				BtnDrawAsStar.Click -= IsStar_Click;
				NumPoints.Leave -= NumPoints_Leave;
				StartAngle.Leave -= StartAngle_Leave;
			}
