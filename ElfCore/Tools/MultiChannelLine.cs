...
{
	[ElfEditTool("Multi-Channel Line")]
	public class MultiChannelLineTool : MultiChannelBase, ITool
	{
		...
		{
			this.ID = (int)ToolID.MultiChannelLine;
		...
		protected override void AttachEvents(bool attach)
		{
			...
			if (attach)
			{
				cboLineThickness.SelectedIndexChanged += new EventHandler(LineThickness_SelectedIndexChanged);
				btnStandardOrder.Click += new EventHandler(StandardOrder_Click);
				btnReverseOrder.Click += new EventHandler(ReverseOrder_Click);
				txtNumChannels.Leave += new EventHandler(NumChannels_Leave);
				txtStartAngle.Leave += new EventHandler(StartAngle_Leave);
				btnSimpleLayout.Click += new EventHandler(SimpleLayout_Click);
				btnDetailedLayout.Click += new EventHandler(DetailedLayout_Click);

				foreach(ToolStripButton Button in _arrangementList)
					Button.Click += new EventHandler(Arrangement_Click);
			}
			else
			{
				cboLineThickness.SelectedIndexChanged -= LineThickness_SelectedIndexChanged;
				btnStandardOrder.Click -= StandardOrder_Click;
				btnReverseOrder.Click -= ReverseOrder_Click;
				txtNumChannels.Leave -= NumChannels_Leave;
				txtStartAngle.Leave -= StartAngle_Leave;
				btnSimpleLayout.Click -= SimpleLayout_Click;
				btnDetailedLayout.Click -= DetailedLayout_Click;
