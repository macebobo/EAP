...
namespace ElfCore.Tools
{
	...
	public class MegaTreeTool : MultiChannelBase, ITool
	{
		...
		protected override void AttachEvents(bool attach)
		{
			...
			if (attach)
			{
				AntiClockwise.Click += new EventHandler(AntiClockwise_Click);
				Clockwise.Click += new EventHandler(Clockwise_Click);
				txtNumLines.Leave += new EventHandler(txtNumLines_Leave);
				txtNumLines.TextChanged += new EventHandler(Settings_TextChanged);
				txtNumLinesPerChan.Leave += new EventHandler(txtNumLinesPerChan_Leave);
				txtNumLinesPerChan.TextChanged += new EventHandler(Settings_TextChanged);
				txtNumChannelsPerLineGroup.Leave += new EventHandler(txtNumChannelsPerLineGroup_Leave);
				txtNumChannelsPerLineGroup.TextChanged += new EventHandler(Settings_TextChanged);
				txtNumRotations.Leave += new EventHandler(txtNumRotations_Leave);
			}
			else
			{
