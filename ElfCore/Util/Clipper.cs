...
namespace ElfCore.Util.ClipperLib
{
	...
	public class ClipperBase
	{
		...
		internal bool SlopesEqual(TEdge e1, TEdge e2, bool UseFullRange)
		{
			if (UseFullRange)
				...
				return (Int64)(e1.deltaY) * (e2.deltaX) ==
	...
	public class Clipper : ClipperBase
	{
		...
		private bool ExecuteInternal()
		{
			...
			if (succeeded)
			{
				...
				foreach (OutRec outRec in m_PolyOuts)
				{
					...
					if (outRec.pts == null)
						continue;
					if (outRec.isHole && m_UsingExPolygons)
		...
		private class PolyOffsetBuilder
		{
				...
				JoinType jointype, double MiterLimit = 2, bool AutoFix = true)
			{
				...
				if (AutoFix)
				{
					...
					while (botI < Len && pts[botI].Count == 0)
						botI++;
					if (botI == Len)
						return;
					...
					{
						if (UpdateBotPt(pts[i][0], ref botPt))
							botI = i;
		...
		public static PointF[] PolygonToPointFArray(Polygon pg, int scale)
		{
		...
		/// <summary>
		/// Convert the GraphicsPath data into the Clipper Polygons object
		/// </summary>
		public static void PathToPolygon(GraphicsPath path, Polygons polys, Single scale)
		{
