...
namespace ElfCore.Util
{
	...
	public class MruStripMenu
	{
		...
		public void AddFile(string filename, string entryname)
		{
			...
			{
				MruMenuItem menuItem = new MruMenuItem(filename, FixupEntryname(0, entryname), OnClick);
				MenuItems.Insert(StartIndex, menuItem);
				SetFirstFile(menuItem);
		...
		public void SaveSettings()
		{
			...
			_settings.SetValue(_settings.AppendPath(SavePath, "Max"), _maxEntries);

			for (int i = 0; i < MenuItems.Count; i++)
			{
				_settings.SetValue(_settings.AppendPath(SavePath, "File" + (i + 1)), ((MruMenuItem)MenuItems[i]).Filename);
			}
			for (int i = MenuItems.Count; i < 16; i++)
			{
				_settings.RemoveValue(_settings.AppendPath(SavePath, "File" + (i + 1)));
			}
			...
			//for (; i < EndIndex; i++, number++)
			//{
			//	_settings.SetValue(_settings.AppendPath(SavePath, "File" + number), ((MruMenuItem)MenuItems[i]).Filename);
			//}

			//for (; number <= 16; number++)
			//{
			//	_settings.RemoveValue(_settings.AppendPath(SavePath, "File" + number));
			//}
		}
		...
		public void LoadSettings()
		{
			...
			MaxEntries = _settings.GetValue(_settings.AppendPath(SavePath, "Max"), 4);

			for (int number = _maxEntries; number > 0; number--)
			{
				filename = _settings.GetValue(_settings.AppendPath(SavePath, "File" + number), string.Empty);
				if (filename.Length > 0)
					AddFile(filename);
			}
