...
{
	[AdjPrevTool("Singing Face")]
	public class SingingFaceTool : MultiChannelBase, ITool
	{
		#region [ Private Variables ]

		// Settings from the ToolStrip
		private int _numChannelsForEyes = 0;
		private int _numChannelsForMouth = 0;

		// Controls from ToolStrip
		private ToolStripComboBox EyeChannels = null;
		private ToolStripComboBox MouthChannels = null;
		...
		public SingingFaceTool() : base()
		{
			this.ID = (int)Tool.SingingFace;
			this.Name = "Singing Face";
			this.ToolBoxImage = ElfRes.singing_face;
			//this.Cursor = CustomCursors.MemoryCursor(ElfRes.cross_singing_face);
			this.Cursor = CreateCursor(ElfRes.cross_base, ElfRes.singing_face_modifier, new Point(15, 15));
		}
		...
		protected override List<GraphicsPath> CreateRenderPathList(Point p1, Point p2, bool finalRender)
		{
			...
			if (finalRender)
			{
				p1 = Workshop.PixelPoint(p1);
				p2 = Workshop.PixelPoint(p2);
			}

			if (finalRender)
				DrawArea = _workshop.NormalizedRectangle(Workshop.CellPoint(p1), Workshop.CellPoint(p2));
			else
			...
			Points[16] = new float[14] { 0.0856f, 0.2310f, 0.3107f, 0.3708f, 0.4308f, 0.5696f, 0.6296f, 0.6897f, 0.6757f, 0.2310f, 0.7497f, 0.7105f, 0.2900f, 0.2507f };
			Points[17] = new float[14] { 0.6238f, 0.2310f, 0.6238f, 0.7123f, 0.6238f, 0.6238f, 0.7123f, 0.6238f, 0.6238f, 0.2310f, 0.7710f, 0.8552f, 0.8552f, 0.7710f };
			...
			// D
			GraphicsPath RightEyeOpen = new GraphicsPath();
		...
		public override void Initialize()
		{
			...
			EyeChannels = (ToolStripComboBox)GetItem<ToolStripComboBox>(1);
			MouthChannels = (ToolStripComboBox)GetItem<ToolStripComboBox>(2);
			Warning_TooFewChannels = (ToolStripLabel)GetItem<ToolStripLabel>(4);

			// Attach events to these controls
			EyeChannels.SelectedIndexChanged += new System.EventHandler(this.EyeChannels_SelectedIndexChanged);
			MouthChannels.SelectedIndexChanged += new System.EventHandler(this.MouthChannels_SelectedIndexChanged);

			// Set the initial value for the contol from what we had retrieve from Settings
			EyeChannels.SelectedIndex = _numChannelsForEyes - 1;
			MouthChannels.SelectedIndex = _numChannelsForMouth - 2;
			
			Warning_TooFewChannels.Visible = false;

			ListBoxUtil.SetDropDownWidth(EyeChannels);
			ListBoxUtil.SetDropDownWidth(MouthChannels);
		}
		...
		private void EyeChannels_SelectedIndexChanged(object sender, EventArgs e)
		{
			_numChannelsForEyes = EyeChannels.SelectedIndex + 1;
			EyeChannels.ToolTipText = EyeChannels.SelectedItem.ToString();
		}

		private void MouthChannels_SelectedIndexChanged(object sender, EventArgs e)
		{
			_numChannelsForMouth = MouthChannels.SelectedIndex + 2; 
			MouthChannels.ToolTipText = MouthChannels.SelectedItem.ToString();
		}
